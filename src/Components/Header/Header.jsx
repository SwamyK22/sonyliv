import React, { useState, useRef } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import './Header.css'
import Profile from '../Profile/Profile';
import MoreDropdown from '../MoreDropdown/MoreDropdown';
import SearchModal from '../searchModal/SearchModal';

function Header() {
const [prePage, setPrePage] = useState('Home');
const ref = useRef();
const nav = useNavigate();
const list = ['Home', 'Movies', 'Tv Shows', 'Sports'];

const openProfile = () => {
  ref.current.style.display = 'block'
}

  return (
    <div className='navbarHeader'>
        <div className='subContainer'>
        <div className='logContainer'>
            <img className='logo' src="https://images.slivcdn.com/UI_icons/sonyliv_new_revised_header_logo.png?h=40&w=40&t=c_fill&q=low&fr=webp" alt="" />
            <button className='subscribeBtn' onClick={() => nav('/subscribe')}>Subscribe &gt;</button>
        </div>
      <div className='pageList'>
        {list.map((x) => (
        <Link className='link' style={x === prePage ? {color:'#ffff', borderBottom: '3px solid #ffff'} : null} onClick={() => setPrePage(x)} to={x === 'Home' ? '/' : x} key={x}>{x[0].toUpperCase()+ x.slice(1) }</Link>
        ))}
      </div>
      <MoreDropdown />
      </div>
      <div className='userContainer'
      >
        
        <SearchModal />
        <img onClick={openProfile} className='user' src="data:image/png;base64," alt="" style={{height:'90%'}} />
      </div>
      <Profile ref={ref}/>
    </div>
  );
}

export default Header;
