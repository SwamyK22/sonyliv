import React, { forwardRef } from 'react'
import { Link } from 'react-router-dom'

const Profile = forwardRef((props, ref) => {

    const closeProfile = () => {
        ref.current.style.display = 'none'
      }

    return (
        <div ref={ref} className='profileContainer' onClick={closeProfile}>
          <Link to='signin' className='section1'>
            <img className='PrfleImg' src='data:image/png;base64,' />
            <p className='Txt'>Sign in</p>
          </Link>
          
          <Link to='subscribe' className='section' >
              <img className='logos' src='https://images.slivcdn.com/UI_icons/Subscribe_Now.png?h=22&w=22&t=c_fill&q=low&fr=webp' />
              <p className='Txt'>Subscribe now</p>
          </Link>
          <div className='section2'>
              <img className='logos' src='https://images.slivcdn.com/UI_icons/New_Final_Icons_30052020/ActivateTV3x.png?h=22&w=22&t=c_fill&q=low&fr=webp' />
              <p className='Txt'>Activate Tv</p>
          </div>
          <div className='section2'>
          <img className='logos' src='https://images.slivcdn.com/UI_icons/New_Final_Icons_30052020/ActivateTV3x.png?h=22&w=22&t=c_fill&q=low&fr=webp' />
              <p className='Txt'>Settings and preferences</p>
          </div>
          <div className='section2'>
          <img className='logos' src='https://images.slivcdn.com/UI_icons/settings_icon.png?h=22&w=22&t=c_fill&q=low&fr=webp' />
              
              <p className='Txt'>Contact Us</p>
          </div>
          <div className='section2'>
          <img className='logos' src='https://images.slivcdn.com/UI_icons/New_Final_Icons_30052020/WhatsApp_icon_2x.png?h=22&w=22&t=c_fill&q=low&fr=webp' />
              
              <p className='Txt'>Chat with us on WhatsApp</p>
          </div>
          <div className='section2'>
          <img className='logos' src='https://images-preprod.slivcdn.com/activate/offer.png?h=22&w=22&t=c_fill&q=low&fr=webp' />
              
              <p className='Txt'>Activate Offer</p>
          </div>
    </div>
    )
})


export default Profile